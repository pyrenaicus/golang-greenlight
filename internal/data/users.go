...
func ValidateUser(v *validator.Validator, user *User) {
	...
	// a useful sanity check to include here, but it's not a problem with the
	// data provided by the client. So rather than adding an error to the
	// validation map we panic instead.
	if user.Password.hash == nil {
		panic("missing password hash for user")
	}
}

// Create a UserModel struct which wraps the connection model.
