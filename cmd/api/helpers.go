...
// readCSV() reads a string value from the query string and then splits it into
// a slice on the comma character. If no matching key is found, return the
// provided default value.
func (app *application) readCSV(qs url.Values, key string, defaultValue []string) []string {
	// Extract the value from the query string.
	csv := qs.Get(key)

	// If no key exist (or the value is empty) return default value.
	if csv == "" {
		return defaultValue
	}
...
// integer before returning it. If no matching key is found, it returns the
// provided default value. If the value cannot be converted to an integer, then
...
func (app *application) background(fn func()) {
	// Use the sync.WaitGroup Go() method to launch the background goroutine.
	app.wg.Go(func() {
		// Recover any panic.
		defer func() {
			pv := recover()
		...
		// Execute the arbitrary function that we passed as parameter.
		fn()
	})
}
